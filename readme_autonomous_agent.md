...
| # | 責務             | 対応モジュール          | 説明                                                                    |
| - | ---------------- | ----------------------- | ----------------------------------------------------------------------- |
| 1 | 実行計画の策定   | `grace/planner.py`      | LLM（Ollama / llama3.2）による計画生成、複雑度推定、フォールバック計画 |
...
flowchart TB
    ...
    subgraph Planner_Layer["Planner Layer (grace/planner.py)"]
        ...
        PlanGeneration["計画生成<br/>(Ollama / llama3.2)"]
    end
...
sequenceDiagram
    ...
    Planner->>Planner: 5. LLM計画生成 (Ollama / llama3.2)
    ...
    loop 各ステップ (Generator)
        ...
        alt rag_search
            ...
            Tools->>Tools: 14. LLM推論 (Ollama / llama3.2)
        end
...
**概要**: ユーザーの質問を分析し、LLM（Ollama / llama3.2）を使って ExecutionPlan を生成する。llama3.2 固有の JSON Schema 参照解決のため `_resolve_schema_refs()` が必要。
...
| **Process** | 1.`_get_available_collections()` — Qdrantから利用可能コレクション一覧を取得<br>2. `estimate_complexity_with_llm(query)` — LLMで複雑度を推定（0.0-1.0）<br>3. `PLAN_GENERATION_PROMPT` にコレクション一覧・クエリを埋め込み<br>4. Ollama OpenAI互換クライアント（`client.chat.completions.create`）で JSON Schema 出力（`_resolve_schema_refs()` でllama3.2向けに $defs 参照を展開）<br>5. `ExecutionPlan.model_validate_json()` でパース<br>6. LLMが推定した複雑度で `plan.complexity` を上書き<br>7. `validate_plan_dependencies()` で依存関係を検証（警告のみ）<br>8. 計画IDを付与（`create_plan_id()`） |
...
| **Process** | 元の計画情報とフィードバックをプロンプトに含め、Ollama OpenAI互換クライアント（`client.chat.completions.create`）で JSON Schema 出力（`_resolve_schema_refs()` でllama3.2向けに $defs 参照を展開）。新しいplan_idを付与 |
...
| `config.GeminiConfig`       | Ollamaモデル設定（利用可能モデル一覧、DEFAULT_MODEL = "llama3.2"） |
...
flowchart TB
    ...
    subgraph External["ローカルサービス（クラウドAPI不要）"]
        Qdrant["Qdrant"]
        Ollama["Ollama API（:11434）<br/>llama3.2"]
...
| `ReasoningTool` | `reasoning`  | LLM推論（検索結果を基にOllama / llama3.2で回答生成）               |
...
| `LLMSelfEvaluator`          | Ollama（llama3.2）による自己評価（query と answer の整合性）                                      |
...
| `MODEL_NAME`             | `GeminiConfig.DEFAULT_MODEL`（= `"llama3.2"`） | デフォルトモデル |
...
| Planner/Executor 初期化エラー            | Ollamaサービス未起動                   | `ollama serve` でOllamaを起動し、`ollama pull llama3.2` でモデルを取得                                         |
