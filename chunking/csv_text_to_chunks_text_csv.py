...
# ----------------------------------------------
# Step1: テキストファイル → チャンク分割、CSV, モデル「gemma4:e4b」は、768次元
#        段落分割 -> 意味的分割 -> 文脈連続性チェック -> 連続性チェック
# ----------------------------------------------
uv run python -m chunking.csv_text_to_chunks_text_csv \
  --input-file OUTPUT/cc_news_2per_768.csv \
...
uv run python qa_qdrant/make_qa_register_qdrant.py \
  --input-file output_chunked/cc_news_2per_768_chunks.csv \
  --collection cc_news_2per_768 \
  --model gemma4:e4b \
  --concurrency 2 \
  --recreate
